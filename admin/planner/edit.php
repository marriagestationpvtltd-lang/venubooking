...
$stmt = $db->prepare("SELECT * FROM event_plans WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || !verifyCSRFToken($_POST['csrf_token'])) {
        ...
        if (empty($title)) {
            ...
            try {
                $stmt = $db->prepare(
                    "UPDATE event_plans SET title=?, event_type=?, event_date=?, customer_id=?,
                     total_budget=?, description=?, status=? WHERE id=?"
                );
                $stmt->execute([
                    $title, $event_type, $event_date, $customer_id,
                    $total_budget, $description, $status, $plan_id
                ]);

                logActivity($current_user['id'], 'Updated event plan', 'event_plans', $plan_id, "Updated plan: $title");

                $_SESSION['success_message'] = 'Plan updated successfully!';
                header('Location: view.php?id=' . $plan_id);
                exit;
            } catch (Exception $e) {
                $error_message = 'Failed to update plan. Please try again.';
            }
        }
    }
...
<div class="card">
    <div class="card-header bg-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0"><i class="fas fa-edit text-warning"></i> Edit Event Plan</h5>
        <a href="view.php?id=<?php echo $plan_id; ?>" class="btn btn-sm btn-outline-secondary">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select" name="status">
                        <?php foreach (['planning' => 'Planning', 'in_progress' => 'In Progress', 'completed' => 'Completed', 'cancelled' => 'Cancelled'] as $val => $label): ?>
                            <option value="<?php echo $val; ?>" <?php echo $plan['status'] === $val ? 'selected' : ''; ?>>
                                <?php echo $label; ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div class="row">
                    ...
                    <select class="form-select" name="event_type" required>
                        ...
                        <?php foreach ($event_types as $et): ?>
                            <option value="<?php echo htmlspecialchars($et); ?>"
                                <?php echo $plan['event_type'] === $et ? 'selected' : ''; ?>>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="date" class="form-control" name="event_date"
                           value="<?php echo htmlspecialchars($plan['event_date'] ?? ''); ?>">
                </div>
                <div class="col-md-4 mb-3">
                    ...
                    <input type="number" class="form-control" name="total_budget" min="0" step="0.01"
                           value="<?php echo htmlspecialchars($plan['total_budget'] ?? '0'); ?>">
                </div>
            </div>
                    ...
                    <select class="form-select" name="customer_id">
                        ...
                        <?php foreach ($customers as $c): ?>
                            <option value="<?php echo (int)$c['id']; ?>"
                                <?php echo (int)($plan['customer_id'] ?? 0) === (int)$c['id'] ? 'selected' : ''; ?>>
