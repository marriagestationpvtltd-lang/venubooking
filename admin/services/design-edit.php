...
$stmt = $db->prepare(
    "SELECT d.*, ss.name AS sub_service_name, ss.service_id, s.name AS service_name
    ...
    if (empty($name) || $price < 0) {
        ...
        try {
            ...
            if (empty($error_message)) {
                $db->prepare(
                    "UPDATE service_designs SET name=?, description=?, price=?, photo=?, display_order=?, status=? WHERE id=?"
                )->execute([$name, $description, $price, $photo_filename, $display_order, $status, $design_id]);
            ...
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <h5 class="mb-0">
                    ...
                    <small class="text-muted ms-2">
                        <a href="view.php?id=<?php echo $design['service_id']; ?>"><?php echo htmlspecialchars($design['service_name']); ?></a>
                        / <?php echo htmlspecialchars($design['sub_service_name']); ?>
                    </small>
                </h5>
                        ...
                        <div class="col-md-8">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="name" name="name"
                                       value="<?php echo htmlspecialchars($design['name']); ?>" required>
                            </div>
                            <div class="mb-3">
                                <label for="price" class="form-label">Price (<?php echo getSetting('currency', 'NPR'); ?>) <span class="text-danger">*</span></label>
                                <input type="number" class="form-control" id="price" name="price"
                                       value="<?php echo htmlspecialchars($design['price']); ?>"
                                       min="0" step="0.01" required>
                            </div>
                            <div class="mb-3">
                                <label for="description" class="form-label">Description</label>
                                <textarea class="form-control" id="description" name="description" rows="3"><?php echo htmlspecialchars($design['description'] ?? ''); ?></textarea>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <?php if (!empty($design['photo'])): ?>
                                    <div class="mb-2">
                                        <img src="<?php echo UPLOAD_URL . htmlspecialchars($design['photo']); ?>"
                                             alt="Current photo" class="img-thumbnail" style="max-height:160px;">
                                ...
                                <div id="photoPreview" class="mt-2" style="display:none;">
                                    <img id="photoPreviewImg" src="" alt="Preview" class="img-thumbnail" style="max-height:160px;">
                                </div>
                            </div>
                            <div class="mb-3">
                                <label for="display_order" class="form-label">Display Order</label>
                                <input type="number" class="form-control" id="display_order" name="display_order"
                                       value="<?php echo intval($design['display_order']); ?>" min="0">
                            </div>
                            <div class="mb-3">
                                <label for="status" class="form-label">Status</label>
                                <select class="form-select" id="status" name="status">
                                    <option value="active" <?php echo $design['status'] === 'active' ? 'selected' : ''; ?>>Active</option>
                                    <option value="inactive" <?php echo $design['status'] === 'inactive' ? 'selected' : ''; ?>>Inactive</option>
                                </select>
                            </div>
                        </div>
                    </div>
