...
$sub_service_id = isset($_GET['sub_service_id']) ? intval($_GET['sub_service_id']) : 0;
if ($sub_service_id <= 0) {
    header('Location: index.php');
    exit;
}

// Verify sub-service exists and get parent service
$stmt = $db->prepare(
    "SELECT ss.*, s.id AS service_id, s.name AS service_name
     FROM service_sub_services ss
    ...
    if (empty($name) || $price < 0) {
        ...
        try {
            ...
            if (empty($error_message)) {
                $stmt2 = $db->prepare(
                    "INSERT INTO service_designs (sub_service_id, name, description, price, photo, display_order, status)
                     VALUES (?, ?, ?, ?, ?, ?, ?)"
                );
                $stmt2->execute([$sub_service_id, $name, $description, $price, $photo_filename, $display_order, $status]);
                $design_id = $db->lastInsertId();
                logActivity($current_user['id'], 'Added design', 'service_designs', $design_id, "Added design '$name' to sub-service '{$sub_service['name']}'");
            ...
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <h5 class="mb-0">
                    ...
                    <small class="text-muted ms-2">
                        for: <a href="view.php?id=<?php echo $sub_service['service_id']; ?>"><?php echo htmlspecialchars($sub_service['service_name']); ?></a>
            ...
            <div class="card-body">
                <?php if ($success_message): ?>
                    <div class="alert alert-success alert-dismissible fade show">
                        ...
                        <a href="view.php?id=<?php echo $sub_service['service_id']; ?>" class="alert-link">Back to service</a>
